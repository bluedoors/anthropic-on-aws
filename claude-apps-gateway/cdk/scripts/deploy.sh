...
# `telemetry.forward_to: @@PUBLIC_URL@@:4318` block (pointed at the ADOT collector
# the stack provisions) only reaches the image through this stamp. The old inline
# heredoc omitted it, so telemetry forwarding was silently off and the collector
# ran idle. Stamping also keeps the model catalog + store wiring in lockstep with
